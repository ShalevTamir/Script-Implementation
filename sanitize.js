...
// ---------------------------------------------------------------------------
// Protected tokens - unlike exclusions (whole files/dirs) or a per-entry
// boundary, this hides an exact literal string (e.g. "nativeElement")
// everywhere it appears before any matching runs, so nothing inside it can
// ever be touched, while a mapping value elsewhere - even as a substring of
// some other word - still matches normally (e.g. "nativ" inside "nativApp"
// still gets renamed; "nativ" inside the protected "nativeElement" doesn't).
// ---------------------------------------------------------------------------

function hideProtectedTokens(text, protectedTokens) {
  let hidden = text;
  const markers = [];
  protectedTokens.forEach((token, i) => {
    if (!token || !hidden.includes(token)) return;
    // NUL-delimited: a text file with any NUL byte would already have been
    // classified binary and never reach content substitution, so this can't
    // collide with real content.
    const marker = `\u0000PT${i}\u0000`;
    hidden = hidden.split(token).join(marker);
    markers.push([marker, token]);
  });
  return {
    hidden,
    restore: (value) => markers.reduce((acc, [marker, token]) => acc.split(marker).join(token), value),
  };
}
...
function renamePaths(root, pairs, protectedTokens) {
  for (const originalPath of collectPathsDeepestFirst(root)) {
    ...
    const { hidden, restore } = hideProtectedTokens(name, protectedTokens);
    const newName = restore(applySubstitution(hidden, pairs));
